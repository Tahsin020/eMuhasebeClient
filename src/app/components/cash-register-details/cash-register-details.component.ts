...
export class CashRegisterDetailsComponent {
  ...
  create(form: NgForm){
    if(form.valid){
      ...
      if (this.createModel.oppositeAmount === 0) {
        this.createModel.oppositeAmount = this.createModel.amount;
      }
  ...
  deleteById(model: CashRegisterDetailModel){
    this.swal.callSwal("Hareketini Sil?",`${model.date} tarihdeki ${model.description} açıklamalı hareketi silmek istiyor musunuz?`,()=> {
  ...
  setOppositeBank(){
    const bank = this.banks.find(p=> p.id == this.createModel.oppositeCashRegisterId)
    if (bank) {
