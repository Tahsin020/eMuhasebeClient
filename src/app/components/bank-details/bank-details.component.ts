...
export class BankDetailsComponent {
  ...
  getAll() {
    this.http.post<BankModel>(
      'BankDetails/GetAll',
      { bankId: this.bankId, startDate: this.startDate, endDate: this.endDate },
      (res) => {
        this.bank = res;
      }
    );
  ...
  deleteById(model: BankDetailModel) {
    this.swal.callSwal(
      'Hareketini Sil?',
      `${model.date} tarihdeki ${model.description} açıklamalı hareketi silmek istiyor musunuz?`,
      () => {
  ...
  setOppositeBank() {
    const bank = this.banks.find(
      (p) => p.id == this.createModel.oppositeBankId
    );
    if (bank) {
