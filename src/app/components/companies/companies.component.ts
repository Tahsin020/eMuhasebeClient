...
export class CompaniesComponent {
  ...
  deleteById(model: CompanyModel) {
    this.swal.callSwal(
      'Şirketi Sil?',
      `${model.name} şirketini silmek istiyor musunuz?`,
      () => {
  ...
  migrateAll() {
    this.swal.callSwal(
      'Databaseleri Güncelle ?',
      'Tüm şirketlerin databaselerini güncellemek üzeresiniz. Devam edilsin mi?',
      () => {
  ...
  seedData(){
    this.swal.callSwal(
      'Seed Data Oluştur ?',
      'Bu şirket için fake veri oluşturmak üzeresiniz. Devam edilsin mi?',
      () => {
